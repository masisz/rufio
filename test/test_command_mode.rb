...
class TestCommandMode < Minitest::Test
  ...
  def test_execute_builtin_hello_command
  ...
  def test_list_available_commands
    ...
    # 組み込みの hello コマンドが利用可能
    assert_includes commands, :hello
    assert_includes commands, :stop
  end
  ...
  def test_command_mode_initialization
    # CommandMode が初期化時に組み込みコマンドを読み込む
    command_mode = Rufio::CommandMode.new
  ...
  def test_builtin_command_still_works_after_shell_support
    command_mode = Rufio::CommandMode.new

    # ! なしの組み込みコマンドも引き続き動作する
    result = command_mode.execute("hello")
    assert_kind_of Hash, result
    assert result[:success]
    assert_includes result[:stdout], "Hello"
